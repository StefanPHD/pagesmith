...
/**
 * Scheibe 2b-i — Server-Side Meta CAPI Forward Route.
 *
 * Erste externe API-Integration der App: unser Server ruft aktiv Metas
 * Graph-CAPI auf. ANONYMER cross-origin Endpoint (der spaetere Beacon aus 2b-ii
 * kommt vom ausgelieferten Export, nicht aus einer Owner-Session). Autorisierung
 * = der oeffentliche trackingKey als CAPABILITY: er loest server-seitig (nur hier,
 * via service_role) auf den geheimen Token auf. Der Token/die Config erreichen die
 ...
 *
 * 2b-ii-LEITPLANKE (hier notiert, NICHT gebaut): der Client-Beacon MUSS ein
 * text/plain-Blob sein (sendBeacon), NIEMALS application/json — application/json
 * macht daraus einen preflight-pflichtigen Request, den sendBeacon (fire-and-forget)
 * nicht bedienen kann -> stiller Ausfall, den auch die CORS-Header unten NICHT
 * retten. Der text/plain-Body ist die tragende Kontrolle, die Header sind nur
