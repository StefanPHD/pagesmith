...
**GEHOBEN AM 2026-08-19, ERSTE RUNDE — DIESER ABSCHNITT IST DAMIT LEER.** Der einzige
Kandidat („EIN ANKER, DER EINDEUTIG AUSSIEHT, IST ES IN EINER DATEI MIT VERZEICHNIS NICHT")
steht seit der Hebung als eigene Regel in `docs/immer-beachten.md` — nicht als Ergänzung an
der WERKZEUG-REGEL, wie der Kandidat vorschlug, sondern eigenständig: dort verfälscht das
WERKZEUG den Gegenstand, hier trifft der ANKER eine andere Stelle als die gemeinte.
DIE ÜBERSCHRIFT BLEIBT STEHEN, obwohl nichts mehr darunter steht — eine leere Überschrift
ist ehrlicher als eine fehlende.
