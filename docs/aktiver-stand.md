...
docs/immer-beachten.md. SEIT DEM SCHNITT VOM 2026-09-08 GILT DAS VERSCHÄRFT: Neben
diesem Verzeichnis führt die Datei DREI REGISTER, und jede Überschrift stünde
mehrfach in ihr, wenn ein Eintrag die Marke trüge.

WAS HIER NICHT MEHR STEHT, WEIL ES AUSGEZOGEN IST — und es ist NICHT verschwunden:
die elf Scheiben-Zuschnitte und die Vermerke liegen in
docs/claude-history/phase-11.2-google.md, der Vorrat in docs/aktiver-stand-vorrat.md.
Beides ist über den Abschnitt "Register — was diese Datei nicht mehr trägt"
auffindbar, je Eintrag mit wörtlichem Titelanfang.

· Pflicht-Gate — diese Datei zuerst
· Gegenstand der Phase
· Was den Zuschnitt bindet
· Fortschreibungs-Regeln
· Die offene Arbeit 1b — der herausgelöste Block aus dem Transport-Zuschnitt
· Register — was diese Datei nicht mehr trägt
...
**WOHER DIESER ABSCHNITT KOMMT:** Der Block darunter stand bis zum 2026-09-08 als
`###`-Abschnitt INNERHALB des Zuschnitts "Der Transport — Scheibe 4 des Schnitts der
Phase 11.2". Beim Schnitt ist er dort herausgelöst worden, weil er als einziges Stück
jenes Zuschnitts NICHT abgelaufen ist: Er beschreibt ausdrücklich NICHT Geschnittenes,
sondern die fünf Vorbedingungen einer noch nicht geschnittenen Arbeit. Der Zuschnitt
selbst liegt in docs/claude-history/phase-11.2-google.md und trägt an der Fundstelle
einen Zeiger.

**DIE `##`-ZEILE DARÜBER IST EINE NEUE KLAMMER UND NICHT DER TITEL DES BLOCKS.** Der Titel
ist die `###`-Überschrift unmittelbar darunter, und sie steht UNVERÄNDERT da — sie wird
aus dieser Datei und aus Vorrats-Eintrag 44 zitiert, und eine Umformulierung machte diese
Zeiger tot. **WER DEN BLOCK SUCHT, SUCHT IHN AN SEINEM EIGENEN TITEL**; wer den Abschnitt
im Verzeichnis sucht, findet die Klammer. Beide Zeilen sind bewusst verschieden
formuliert, damit ein Such-Anker nicht beide trifft.

**AM INHALT IST NICHTS GEÄNDERT**, auch nicht die Angabe "nicht geschnitten" im Titel: Sie
ist für 1b-1 und 1b-2a überholt, und der Block sagt das in seinen eigenen zwei Zeigern von
2026-09-03 selbst. **DIE ZUSCHNITTE WERDEN VERSCHOBEN, NICHT BEURTEILT** — das ist die
Auflage aus "Die Grenze dieses Zuschnitts" weiter unten, und sie gilt hier genauso.
...
**WOZU SIE DA SIND:** Diese Datei ist am 2026-09-08 in drei geteilt worden. Ohne die
Register wüsste eine Sitzung, die nur sie liest, nicht einmal, DASS es etwas nachzusehen
gibt — und dann wäre die Teilung eine blosse Verlagerung des Problems statt einer Lösung.

**BAUFORM, wie das Verzeichnis in docs/immer-beachten.md:** je Eintrag der WÖRTLICHE
ANFANG, auf feste Breite geschnitten und mit " ..." gekappt, wo er länger war. **KEINE
BESCHREIBUNG** — eine Beschreibung wäre eine zweite Wahrheit, die neben dem Eintrag
altert und irgendwann etwas anderes sagt als er; ein wörtliches Zitat lässt sich per
Suche gegen den Eintrag prüfen und kann nicht auseinanderlaufen.

**DREI AUFLAGEN AN DIE FORM, alle drei aus bestehenden Regeln und keine aus diesem
Schnitt:**
· **KEIN Register-Eintrag trägt eine `##`- oder `###`-MARKE.** Das folgt aus "EIN ANKER,
  DER EINDEUTIG AUSSIEHT, IST ES IN EINER DATEI MIT VERZEICHNIS NICHT"
  (docs/immer-beachten.md). Trüge er sie, stünde jede Überschrift mindestens zweimal in
  dieser Datei, und eine Überschriften-Suche träfe systematisch zuerst das Register. Die
  DREI Register vervielfachen diese Gefahr gegenüber dem einen Verzeichnis im Kopf.
· **DIE AUSZEICHNUNGS-MARKEN `**` UND `` ` `` SIND IN DEN AUSZÜGEN ENTFERNT.** Ein
  gekappter Auszug hinterliesse sonst eine offene Marke, und die färbte den Rest des
  Registers. **Am Wortlaut ändert das nichts** — entfernt sind Zeichen der Auszeichnung,
  kein Wort.
· **KEINE STÜCKZAHL IN DEN REGISTER-ÜBERSCHRIFTEN, UND ES KOMMT KEINE HINEIN.** Es ist
  dieselbe Bauform, die in dieser Datei bereits zweimal ausdrücklich verboten ist (s. die
  Köpfe von "Entscheidungen, die über ihre Scheibe hinaus binden" und — in
  docs/aktiver-stand-vorrat.md — des Vorrats): Vermerke und Vorrats-Einträge sind
  nummeriert, die Listen zählen sich damit selbst, und eine Zahl daneben wird bei jedem
  Zuwachs neu falsch. **DIE ZUSCHNITTE SIND NICHT NUMMERIERT** — das Register spiegelt
  das und erfindet keine Nummern; wer sie zählen will, zählt die Zeilen.

**FORTSCHREIBUNG:** Ein neuer Eintrag tritt HINTEN an, im Register wie in der Zieldatei.
Nichts wird umsortiert, nichts nachnummeriert. **WER EINEN ABSCHLUSS-VERMERK SCHREIBT,
ZIEHT REGISTER 2 IM SELBEN ZUG NACH** — sonst wächst das Archiv, und nichts in dieser
Datei sagt es.

### Register 1 — die Zuschnitte

Die Volltexte liegen in docs/claude-history/phase-11.2-google.md.

· Scheibe 11.2a — Klick-Kennungen lösen und die Nutzlast ...     → phase-11.2-google.md
· Die Erneuerung des Zugangsdatums — Scheibe 1a des ...          → phase-11.2-google.md
· Google als reguläres Ziel in der Oberfläche — Scheibe 3 ...    → phase-11.2-google.md
· Die Rückkehr in das gestartete Projekt — eine mitgereiste ...  → phase-11.2-google.md
· Die Konto-Kennungen bekommen ihre Eingabe — Scheibe 2 des ...  → phase-11.2-google.md
· Der Transport — Scheibe 4 des Schnitts der Phase 11.2          → phase-11.2-google.md
· Die Klammer um die Erneuerung — Schritt 1b-1 der Scheibe ...   → phase-11.2-google.md
· Die Rettung am Beacon — Scheibe 1b-2a des Schritts 1b-2 ...    → phase-11.2-google.md
· Die Ampel an der Ziel-Karte — Scheibe 11.2b                    → phase-11.2-google.md
· Der Riegel gegen die verlorene Schreibung — Scheibe 1b-2b ...  → phase-11.2-google.md
· Der Schlüssel kommt aus der Spalte — eine zweite ...           → phase-11.2-google.md

### Register 2 — die Vermerke

Die Volltexte liegen in docs/claude-history/phase-11.2-google.md. Nummer und Bau-Commit
stehen so, wie sie in der Überschrift des jeweiligen Vermerks stehen.

· 1 (Commit 2d0b59e) — DIE GESTALT-ENTSCHEIDUNG UND DIE MESSUNG DER KLICK-KENNUNG
· 2 (Commit 6653f37) — SCHEIBE 11.2a IST GEBAUT
· 3 (Commit 18d0ff5) — DER TRÄGER DES ZUGANGSDATUMS IST GEMESSEN, DIE SPERRE IST ...
· 4 (Commit abcff79) — DIE NUTZLAST IST GEMESSEN, DIE WIRE-FORM IST KEINE DOKU-LESUNG ...
· 5 (Commit 4aba869) — DIE ERNEUERUNG IST GEMESSEN, DAS ERNEUERUNGS-TOKEN ROTIERT ...
· 6 (Commit a351858) — DIE SCHEIBE 1a IST GEBAUT UND LIVE BEWIESEN
· 7 (Commits 659d672, 7771019, aa17f11) — DIE SCHEIBE 3 IST GEBAUT UND LIVE BEWIESEN
· 8 (Commit 2b735aa) — DIE FIX-SCHEIBE IST GEBAUT UND LIVE BEWIESEN
· 9 (Commit 6dc7e27) — DIE SCHEIBE 2 IST GEBAUT UND LIVE BEWIESEN
· 10 (Bau-Commits 26caa38, 84e9fca) — DIE SCHEIBE 4 IST GEBAUT UND LIVE BEWIESEN
· 11 (Bau-Commit 6bc01ed) — DER SCHRITT 1b-1 IST GEBAUT UND LIVE GEPRÜFT
· 12 (Bau-Commit d57d50c) — DIE SCHEIBE 1b-2a IST GEBAUT UND LIVE BEWIESEN
· 13 (Bau-Commit 7288f90) — DIE SCHEIBE 11.2b IST GEBAUT UND LIVE BEWIESEN
· 14 (Bau-Commit 2eae9ca) — DIE SCHEIBE 1b-2b IST GEBAUT UND LIVE BEWIESEN
· 15 (Bau-Commit bf73b85) — DIE SCHEIBE "Der Schlüssel kommt aus der Spalte" IST ...
· 16 (Bau-Commits 57c9231, eeac5dd, 25119b7) — DIE TEILUNG DER STANDDATEI IST ...

### Register 3 — der Vorrat

Die Volltexte liegen in docs/aktiver-stand-vorrat.md. **NICHT im Archiv** — der Vorrat
ist nicht abgelaufen und wird am Phasenende GEHOBEN, nicht archiviert.

· 1. DER EINWILLIGUNGS-DRAHT FÜHRT KEINEN GOOGLE-SCHLÜSSEL. GEMESSEN AM CODE (CC ...
· 2. GENAU EINE STELLE IM REPO ZERLEGT EINEN QUERY-STRING, UND ES IST DIE EIGENE. DIE ...
· 3. EIN ADAPTER HAT KEINEN RÜCKKANAL — AUF ZWEI EBENEN, NICHT AUF EINER. GEMESSEN am ...
· 4. DIE SCHREIBUNG DER URL-PARAMETERNAMEN STÜTZT SICH AUF NICHTS GELESENES. GEBAUT ...
...
· 10. retry HAT KEINE OBERGRENZE, UND SCHEIBE 1b MUSS EINE LIEFERN. DREI Ausgänge der ...
...
· 17. DIE GIT-WARNUNG WAR DER AUSLÖSER, NICHT DIE KONTROLLE. DER VORFALL (CC ...
· 18. connectOutcome HÄNGT AM ZWEIG "KEIN GEHEIMNIS-FELD", NICHT AM ZIEL. Die Karte ...
· 19. DER RESET LEERT DIE MELDUNG AUCH BEI EINEM ANDEREN ZIEL. Ein Google-Fehlercode ...
· 20. DIE MUTATIONS-VORHERSAGE 1 WAR ZU ENG — DER SECHSTE PROTOKOLLIERTE FALL, DER ...
· 21. VORRATS-EINTRAG 19 WIRD DURCH DIE FIX-SCHEIBE SCHÄRFER, NICHT GEGENSTANDSLOS. ...
· 22. KORREKTUR AN VORRATS-EINTRAG 17 — DIE ZAHL, NICHT DER BEFUND. DIE MESSUNG (CC ...
· 23. — DIE NUMMER IST FREI UND BLEIBT ES. Hier stand am 2026-08-31 kurzzeitig "DIE ...
· 24. PROJECT_PARAM STEHT ZWEIMAL, UND DIE DIVERGENZ IST EINSEITIG STUMM. DER BEFUND ...
· 25. loadProject EBNET DREI FÄLLE AUF null EIN. GEMESSEN am Code (CC, 2026-08-31) ...
· 26. EINE BESTANDS-LINT-WARNUNG IN src/lib/tracking/consent.test.ts. GEMESSEN (CC ...
· 27. DER BELEG AN settingsEqual TRÄGT NICHT MEHR — DIE REGEL BLEIBT WAHR. DER ...
· 28. DER SCHRÄGSTRICH FÄLLT NICHT — UND DAS IST KONFORM, KEIN DEFEKT. GEMESSEN LIVE ...
· 29. EINE GRENZE AM ZEICHENVORRAT DER NORMALISIERUNG. [-\s] trifft den ...
· 30. DER never-KOMMENTAR IN TargetCard.test.tsx IST ÜBERHOLT — UND WAR ES SCHON VOR ...
· 31. DAS aria-label-MUSTER AN DER EREIGNIS-ACHSE IST DATEN, KEIN MECHANISMUS. DER ...
· 32. VERMERK AN VORRATS-EINTRAG 20 — DIE URSACHE, NICHT DIE ZAHL. Jener Eintrag hält ...
· 33. DAS INSTRUMENT ZUM AUFFINDEN EINES NUL-BYTES IST EIN ANDERES ALS DAS ZUM SUCHEN ...
· 34. EIN ZEILENWEISER KOMMENTAR-FILTER IST BEI MEHRZEILIGEN BLÖCKEN UNTAUGLICH. Er ...
· 35. DER KOMMENTARKOPF VON schedulePersist DECKT DEN CODE NICHT. GEMESSEN am Code ...
· 36. DIE ADAPTER REICHEN EINE KLICK-KENNUNG HEUTE SCHON DURCH — AUF EINER ANDEREN ...
· 37. DER else-ZWEIG IN TargetCard.test.tsx HAT SEIT SCHEIBE 4 KEINEN FALL MEHR — UND ...
· 38. DER KOPF DES GOOGLE-ABSCHNITTS IN docs/ziel-befunde.md SAGT "NICHTS IST ...
· 39. CONVERSIONS AUF FOLGESEITEN SIND FÜR GOOGLE HEUTE NICHT MESSBAR — UND DIE ...
· 40. EINE MESSUNG WURDE MIT AUSGELASSENEM SCHLÜSSELWERT ABGELEGT — UND DER ...
· 41. INVALID_GCLID VERDECKT DIE TAG-HYPOTHESE — SIE IST WEDER BESTÄTIGT NOCH ...
· 42. DER RESOLVER SCHREIBT BEI TOTEM ZUGANGSDATUM EINE FEHLERZEILE JE BESUCHER ...
· 43. "STUMM" GILT FÜR DIE OBERFLÄCHE, NICHT FÜR DEN BETRIEB. Diese Datei sagt an ...
· 44. DIE BEWEIS-ROUTE HAT KEINEN AUFRUFER IN DER ANWENDUNG. Diese Datei sagt an zwei ...
· 45. DER BEREICH "VERÖFFENTLICHEN" FÜHRT BEI EINEM PROJEKT MIT VERBUNDENER ...
· 46. DIE NUTZLAST VON HANDAUFRUF 3 DER MESSUNG D IST NUR RELATIONAL PROTOKOLLIERT ...
· 47. DAS TOKEN "1b" IST IM REPO NICHT EINDEUTIG — ZWEI NUMMERIERUNGEN TEILEN DIE ...
· 48. DIE FEHLERZEILE IST MEHRDEUTIG GEWORDEN — DERSELBE WORTLAUT UND DIESELBE ...
· 49. DIE BENENNUNG EINES TESTLAUFS IST WEITER ALS DIE SACHE — "PageView rettet ...
· 50. STIRBT DAS ERNEUERUNGS-TOKEN, IST DER AUSFALL FÜR NIEMANDEN SICHTBAR. DER ...
· 51. cut -c<n> SCHNEIDET NACH ZEICHEN UND ZERLEGT DABEI MEHRBYTE-ZEICHEN — DIE FOLGE ...
· 52. DREI COMMITTETE ABSCHNITTE TRAGEN DENSELBEN SAMMEL-PROVENIENZ-SATZ — UND AN ...
...
· 55. ZWEI WERTE IM EINSTELLUNGS-BLOB WERDEN GEPFLEGT UND VON KEINEM KONSUMENTEN ...
· 56. EINE WARNZEILE IM AUSGELIEFERTEN TEXT SAGT "NICHT KONFIGURIERT" UND STEHT ...
· 57. EIN BESTANDSLAUF VERGLEICHT ZWEI WEGE GEGENEINANDER UND GEHT AUCH DANN AUF ...
· 58. DAS INSTRUMENT DER PRÜFUNG HAT DEN ZUSTAND VERÄNDERT, DEN ES PRÜFEN SOLLTE ...
...
**DER GEGENSTAND, IN EINEM SATZ:** Diese Datei WAR **680 598 Bytes und 9 637 Zeilen** gross
...
LIEST, IST KEIN GATE** — es sieht bei jeder Sitzung genauso erfüllt aus wie beim ersten
Mal. Dieselbe Datei ist zugleich der MASSSTAB, gegen den jede Verdichtung misst; sie lässt
sich also nicht einfach kürzen.
...
Register mit **10 349 B — GEMESSEN (CC, 2026-09-08) am gebauten Abschnitt**; hier stand bis
dahin "rund 10 000 B — SCHÄTZUNG DES ARCHITEKTEN (2026-09-07), UNGEMESSEN", und die Schätzung
lag um 349 B daneben. **DAMIT IST KEIN POSTEN DIESER RECHNUNG MEHR GESCHÄTZT.** Die ADDITION
kommt auf **rund 144 000 B** — **GEMESSEN IST DIE STEUERDATEI NACH DEM SCHNITT ABER 148 817 B**
(CC, 2026-09-08; derselbe Wert steht im Gegenstands-Satz ganz oben). **ES GILT DIE MESSUNG, NICHT
DIE ADDITION**, und die Differenz von rund 5 000 B wird benannt statt weggerundet: Sie ist der
Text, den der Schnitt selbst erzeugt hat und den die Addition nicht kennen konnte — die Köpfe der
drei Register, die Klammer um "1b als Folgetask", die Herkunfts-Zeilen der zwei umgezogenen
Blöcke und die nachgezogenen Zeilen dieses Abschnitts. **EIN POSTEN DER ADDITION IST DABEI SELBST
ÜBERHOLT:** die "rund 21 000 B für diesen Abschnitt" eine Zeile höher sind mit diesem Nachzug
gewachsen; sie stehen aus dem Grund gerundet da, den der nächste Absatz nennt, und werden nicht
zum dritten Mal nachgezogen. Das EINZIGE Nicht-Gemessene dieses Abschnitts ist entfallen;
die frühere Auflage, die 10 000 B nicht als Messwert weiterzureichen, ist gegenstandslos.
...
· **"Die Entscheidungen vom 2026-08-29 — ACHT, und sie binden über diese Scheibe
  hinaus"** (4 907 B) — im Zuschnitt "Die Erneuerung des Zugangsdatums — Scheibe 1a".
· **"Drei Entscheidungen, die nach den sechs Festlegungen gefallen sind — sie
...
— er ist bei der Verdichtung der Scheibe 1a entfallen und wird in
docs/claude-history/phase-11.2-google.md als entfallen zitiert, im Zuschnitt **"Die
Erneuerung des Zugangsdatums — Scheibe 1a des Schnitts der Phase 11.2"** unter
**"Vollzogen — was hier stand und wohin es gegangen ist"**. **DER ZUSCHNITT-TITEL GEHÖRT
ZUR ADRESSE UND IST KEINE HÖFLICHKEIT:** Jener Unterabschnitt-Titel steht in jener Datei
ZWEIMAL ZEICHENGLEICH, und eine Suche allein danach trifft systematisch den falschen von
beiden. GEMESSEN am Repo (CC, 2026-09-07). Beide Zeiger laufen bei einer
...
VERDICHTET AM 2026-09-08, nach den Bau-Commits 57c9231, eeac5dd und 25119b7 und dem
bestätigten Vollzug. Hier standen die ANWEISUNGEN FÜR die Teilung; sie sind mit dem Vollzug
abgelaufen. **DIE ENTSCHEIDUNGEN UND DIE GRENZE BLEIBEN** — der Abschnitt hat das für sich
selbst so festgelegt, und diese Verdichtung hält sich daran.

VIER Stücke sind entfallen. **DIE TITEL SIND OHNE ###-MARKE ZITIERT**, damit das Zitat nicht
dauerhaft mit jeder künftigen Überschrift desselben Wortlauts kollidiert:

- **"Die drei Schritte der Teilung, und warum die Zeiger zuletzt kommen"** nannte Schritt 1,
  2 und 3 und begründete ihre Reihenfolge. Alle drei sind gefahren. **SEINE ZWEI
  FORTWIRKENDEN EINSICHTEN STEHEN IN VERMERK 16, Abschnitt (e)** — dass vor Schritt 2
  niemand weiss, wohin die Zeiger zeigen, und dass der Zwischenzustand nach Schritt 2 LESBAR
  falsch ist, der nach einem vorgezogenen Schritt 3 aber nicht. **Nur weil sie dort stehen,
  durften sie hier weg;** beide standen sonst nirgends im Repo (GEMESSEN, CC, 2026-09-08).
- **"Was in Schritt 2 an docs/arbeitsweise.md und CLAUDE.md nachzuziehen ist"** führte SECHS
  Stellen in docs/arbeitsweise.md und DREI in CLAUDE.md. **Alle neun sind in eeac5dd
  nachgezogen**, im selben Commit wie der Schnitt; die Liste war eine Arbeitsanweisung und
  hat ihren Gegenstand verloren. Der Satz "KEINE NEUE DATEI OHNE OWNER-ENTSCHEIDUNG ist
  beachtet" fällt mit ihr — er steht als Regel in CLAUDE.md und wird hier nicht verdoppelt.
- **"WAS DIESE RUNDE NICHT GETAN HAT"** beschrieb, was Schritt 1 bewusst unterliess. Schritt
  2 hat genau das dann getan; als Aussage über den heutigen Zustand war der Absatz
  irreführend, als Zeitdokument über den 2026-09-07 war er richtig.
- **"ER BLEIBT IN DER STEUERDATEI ... Nach Vollzug wird er verdichtet wie jeder andere
  Zuschnitt"** ist mit dieser Verdichtung eingelöst. Er bleibt in der Steuerdatei; das sagt
  jetzt der Ort, nicht mehr der Satz.

WAS BEWUSST NICHT VERDICHTET WORDEN IST, je mit seinem Grund — im Zweifel stehengelassen:
- **Die Byte-Aufstellung und die Addition** bleiben: Sie sind der MASSSTAB, gegen den die
  nächste Verdichtung misst, und sie tragen die einzige Herleitung der Grössenordnung.
- **"Die elf Entscheidungen, die heute am falschen Ort stehen"** bleibt, obwohl der Umzug
  vollzogen ist: Der Abschnitt trägt die offene Grenze "OB ES BEI ELF BLEIBT, IST NICHT
  GEPRÜFT" und die Lektion über den stillen Schaden eines mechanischen Schnitts.
- **"Der Preis: 27 Zeiger, die auswandern"** bleibt als Zeitdokument. **Seine Zahl ist
  überholt** — gemessen sind 20 (VERMERK 16, Abschnitt (d)) —, und er nennt sich selbst eine
  untere Schranke und ausdrücklich keinen Prüfumfang.
- **"Die Grenze dieses Zuschnitts"** bleibt vollständig: Ihre Auflage ist gefahren, aber ihr
  Befund — die Festlegungs-Blöcke der elf Zuschnitte sind ungeprüft — steht offen.
...
### Wo dieser Zuschnitt liegt

**ER BLEIBT IN DER STEUERDATEI** und ist nicht ins Archiv gewandert — ein Zuschnitt, der die
Teilung anordnet, darf nicht mit dem Block wandern, den er anordnet.
...
**ARCHITEKTEN-SETZUNG (2026-09-07)** und revidierbar. Die Byte-, Zeilen- und Zeigerzahlen der
PLANUNG sind **GEMESSEN am Repo (CC, 2026-09-07)** und stammen aus dem Messbericht desselben
Tages; die Summen 102 590 B, 112 443 B und die rund 144 000 B sind ADDITIONEN dieser Werte und
keine eigenen Messungen. **VIER ANGABEN SIND JÜNGER UND TRAGEN IHR EIGENES DATUM:** die
10 349 B der Register und die Grösse der geschnittenen Datei (**GEMESSEN, CC, 2026-09-08**),
die vier /context-Werte des Ladbarkeits-Blocks (**GEMESSEN, OWNER, /context, 2026-09-08**) und
die Verdichtung dieses Abschnitts (**CC, 2026-09-08**). **KEINE ANGABE DIESES ABSCHNITTS IST
MEHR GESCHÄTZT** — die frühere Architekten-Schätzung von rund 10 000 B ist oben ersetzt. Keine
Messung an einer laufenden Anwendung, keine Anbieter-Lesung.
